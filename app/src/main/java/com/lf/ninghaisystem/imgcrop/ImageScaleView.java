...
//        return super.onTouchEvent(event);
    ...
    public Bitmap getCropImage() {
        int imgHeight = (getWidth() - 2 * mLeftPadding);
        if (CropBorderView.borderOption != null) {
            switch (CropBorderView.borderOption) {
                case TWO2THREE:
                    imgHeight = imgHeight / 2 * 3;
                    break;
                case THREE2TWO:
                    imgHeight = imgHeight / 3 * 2;
                    break;
                case FIVE2EIGHT:
                    imgHeight = imgHeight / 15 * 8;
                    break;
                case ONE2ONE:
                default:
                    break;
        ...
        this.mTopPadding = (getHeight() - imgHeight) / 2;
        Bitmap bitmap = null;
        bitmap = Bitmap.createBitmap(getWidth(), getHeight(), Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(bitmap);
        draw(canvas);
        bitmap = Bitmap.createBitmap(bitmap, mLeftPadding, mTopPadding, getWidth() - 2 * mLeftPadding, imgHeight);
        return bitmap;
    }
}
